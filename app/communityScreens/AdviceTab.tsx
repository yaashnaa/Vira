...
import TabInfoCard from "@/components/community/TabInfoCard";

export default function AdviceTab() {
  interface Post {
    id: string;
    name: string;
    [key: string]: any; // Adjust based on the actual structure of your posts
  }
  ...
  useEffect(() => {
    const unsubscribe = listenToPosts("Advice", setPosts);
    return unsubscribe;
  }, []);

  const toggleComments = (postId: string): void => {
    setVisibleComments((prev: Record<string, boolean>) => ({
      ...prev,
      [postId]: !prev[postId],
    }));
  };
  ...
  return (
    <View style={styles.container}>
        ...
        renderItem={({ item }) => (
          <View style={styles.postBlock}>
            ...
            {visibleComments[item.id] && (
              <CommentSection category="Advice" postId={item.id}  onCommentPosted={() => toggleComments(item.id)}/>
            )}
          </View>
        )}
      ...
      <Button
        mode="contained"
        onPress={async () => {
          await createPost("Advice", newPost);
          setNewPost("");
        }}
        style={styles.button}
      >
